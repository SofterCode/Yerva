...
//        }
        ...
        int count=-1;
        String parkName = "";
        final Integer numOfLots = Ipsum.FacilitiesPLots.size();
        Double parkLat = 0.0;
        Double parkLon = 0.0;
        for(Object lot : Ipsum.FacilitiesPLots){
            count++;
             parkName = Ipsum.FacilitiesPLots.elementAt(count).getParkName().toString();
            parkLat = Ipsum.FacilitiesPLots.elementAt(count).getParkLat();
            parkLon = Ipsum.FacilitiesPLots.elementAt(count).getParkLong();
            ...
            mMap.setOnInfoWindowClickListener(new GoogleMap.OnInfoWindowClickListener() {
                ...
                public void onInfoWindowClick(Marker arg0) {
                    ...
                    for(int i =0;i < numOfLots; i++){
                        if(Ipsum.FacilitiesPLots.get(i).getParkName().contains(arg0.getTitle())){
                            index = i;
                        }
                        send.putExtra("Name", Ipsum.FacilitiesPLots.get(index).getSampleId());
                        index =0;
                        startActivity(send);
                    }
                }
